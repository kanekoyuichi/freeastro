...
| `position` | `294.70` | 黄道経度（0〜360の絶対値）。通常は `sign` + `sign_degree` で十分です。 |
| `house` | `9` | ハウス番号（1〜12） |
...
| `position` | `43.12` | 黄道経度（0〜360の絶対値）。通常は `sign` + `sign_degree` で十分です。 |
...
| `planet1` | `"Moon"` | 天体1 |
| `planet2` | `"Mercury"` | 天体2 |
| `aspect` | `"Trine"` | アスペクトの種類 |
| `angle` | `119.09` | 天体間の実際の角度差（0〜180） |
| `orb` | `-0.91` | 正確な角度からのずれ（Trine = 120° なので 119.09 − 120 = −0.91） |
